...
async function fetchForecastData(targetDate) {
    try {
        ...
        const gru = (data.model_diagnostics || {}).gru || {};
        const gruMetrics = gru.metrics || {};
        const gruStatus = {
            ok: "Đã bật và đã backtest",
            disabled: "Đang tắt (bật MEAL_HISTORY_ENABLE_GRU=1)",
            insufficient_data: "Chưa đủ dữ liệu để huấn luyện",
        }[gru.status] || "Không khả dụng";
        document.getElementById("gru-status").innerText =
            `Trạng thái: ${gruStatus}; mô hình ${data.model === "GRU" ? "đang được chọn" : "chưa được chọn"}.`;
        document.getElementById("gru-lookback").innerText = gru.lookback ?? "-";
        document.getElementById("gru-hidden-size").innerText = gru.hidden_size ?? "-";
        document.getElementById("gru-epochs").innerText = gru.epochs ?? "-";
        document.getElementById("gru-learning-rate").innerText = gru.learning_rate ?? "-";
        document.getElementById("gru-parameters").innerText = gru.parameters ?? "-";
        document.getElementById("gru-training-samples").innerText = gru.training_samples ?? "-";
        document.getElementById("gru-validation-samples").innerText = gru.validation_samples ?? "-";
        document.getElementById("gru-loss").innerText = gru.loss ?? "-";
        document.getElementById("gru-mae").innerText = gruMetrics.mae ?? "-";
